...
from flask import render_template, flash, redirect, url_for, request, make_response, jsonify, abort
...
def post_temperaturereadings():
	...
	r = TemperatureReading(particleid=reading['particleid'], temperature=reading['temperature'])
	db.session.add(r)
	db.session.commit()
...
def get_temperaturereading(particleid):
	...
	try:
		readingsdb = TemperatureReading.query.filter_by(particleid=particleid).order_by(TemperatureReading.timestamp.desc()).paginate(1,1000,False).items
	except Exception as e:
		return("type error: " + str(e))
	
	try:
		for i in readingsdb:
			if i.temperature > -20 and i.temperature <50: 	# My sensor occasionally returns ridiculous values. Throw those out.
...
def post_MultiSensorReadings():
	...
	try:
		r = MultiSensorReading(particleid=reading['particleid'], sensor1=reading['sensor1'], sensor2=reading['sensor2'], sensor3=reading['sensor3'], sensor4=reading['sensor4'], sensor5=reading['sensor5'])
		db.session.add(r)
		db.session.commit()
	except Exception as e: 
...
def get_MultiSensorReadings(particleid):
	...
	try:
		readingsdb = MultiSensorReading.query.filter_by(particleid=particleid).order_by(MultiSensorReading.timestamp.desc()).paginate(1,1000,False).items
	except Exception as e:
		return("type error: " + str(e))
	try:
		for i in readingsdb:
